...
if [ -n "${HOSTS_JSON:-}" ] && echo "$HOSTS_JSON" | jq -e . >/dev/null 2>&1; then 
    ...
    if [ -n "${USE_OWN_DNS:-}" ]; then
        while true; do
            IP=$(dig +short challenge.pccc | head -n1)
            ...
            IP=$(dig +short challenge.us | head -n1)
...
# Start the scanner application in the background if it exists
if [ -f /app/scanner/app.py ]; then
    echo "[entrypoint] Starting scanner application on port 5555..."
    cd /app/scanner && python app.py &
fi
