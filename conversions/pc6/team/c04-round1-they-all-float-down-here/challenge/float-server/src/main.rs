...
use log::{error, info};
use rocket::{http::ContentType, serde::json::Json, Config};
...
impl Part {
    ...
    fn get_part_token(&self) -> Result<String, ()> {
        match env::var(self.env_var_name()) {
            ...
            Ok(_) => {
                error!("Environment variable {} is empty.", self.env_var_name());
                Err(())
            }
            Err(e) => {
                error!(
...
const CURL_COMMAND_SH: &str = r#"#!/bin/bash
...
#[get("/curl_command.sh")]
fn get_curl_command() -> (ContentType, &'static str) {
    (ContentType::new("application", "x-shellscript"), CURL_COMMAND_SH)
}
...
fn rocket() -> _ {
    ...
    rocket::custom(&config).mount("/", routes![part1, part2, part3, part4, get_curl_command, get_schema])
}
