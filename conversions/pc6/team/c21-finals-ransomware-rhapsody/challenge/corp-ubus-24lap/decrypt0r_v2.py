        ...
        with open("/proc/self/status", "r") as f:
            for line in f:
                if "TracerPid" in line:
                    ...
                    if tracer_pid != 0:
                        print("That would be a no-no. Debugger detected! Exiting...")
                        sys.exit(1)
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("No funny stuff when entering an IV. Pay attention to formatting")
        exit(0)

    try:
        decrypted_dir = input("Enter the directory to save decrypted files without ending slash: ")
    except Exception as e:
        print('No funny stuff in naming a save spot. GET SERIOUS. W3 W4NT YOU')
        exit(0)
