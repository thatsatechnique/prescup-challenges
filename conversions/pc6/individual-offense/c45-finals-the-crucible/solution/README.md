...
**The Crucible** places you in the role of an operative tasked with uncovering hidden intelligence across a compromised network. A mysterious signal from honor.pccc hints at a locked message, protected by a PIN authentication system. A cryptic image on the same server leads to oblivion.pccc, where covert UDP communications hide encrypted transmissions between two unknown entities. To intercept their exchange, you must launch a man-in-the-middle attack, decrypt their protocol, and extract a secret key. Your final challenge takes you to champion.pccc.  To get there, you must reconstruct a corrupted SSH private key which holds the last piece of the puzzle — requiring forensic recovery and cryptographic repair to gain access.

## Question 1
Token 1: _After finding the correct pin, what is the token hosted by honor.pccc (in token.txt)?_
...
Nmap scan report for challenge.pccc (10.5.5.5)
...
Nmap scan report for honor.pccc (10.5.5.78)
...
Nmap scan report for oblivion.pccc (10.5.5.129)
...
Nmap scan report for champion.pccc (10.5.5.130)
...
Nmap scan report for phantom.pccc (10.5.5.133)
...
Nmap scan report for kali-crucible.pccc (10.5.5.74)
...
Going through the list, we can first immediately recognize ourselves at `kali-crucible.pccc` and the off limits `challenge.pccc` server that is used for grading in other challenges. We can safely ignore both of these. That leaves four servers of interest to us.
The `honor.pccc` server has an `HTTP` service on port `80` and an unknown service on port `61234` (very suspicious!) The remaining servers are `oblivion.pccc`, `champion.pccc`, and `phantom.pccc`, all of which appear to only have an SSH port open at the moment. If you'd like, you can try to SSH into those servers (e.g., `ssh user@phantom.pccc`), but you'll find those only allow connection via private key, which we do not have.

If you reread the questions, which were fairly vague in the beginning, they now begin to make sense. Each question is hinting at a server. Question 1, our goal for now, mentions Honor, so we should turn our attention to `honor.pccc` for now.

Let's begin by inspecting the web server on `honor.pccc`. Opening Firefox and visiting `honor.pccc` simply gives a directory listing that contains only a single image named `oblivion.jpg`. That matches the name of one of our other servers, and is thus likely connected to the `oblivion.pccc` server. If you would like to begin investigating this image, jump to [Question 2](#question-2); otherwise, let's take a look at that suspicious port `61234`.

![A screenshot of Firefox with the URL honor.pccc entered; the page is a simple, default directory listing reading "Index of /". The only file listed is an image file named "oblivion.jpg".](./imgs/honor-first-listing.png "Directory Listing on honor.pccc")
...
![A screenshot of a Kali terminal running the previous netcat command to connect to the honor server. Random PINs have been entered such as 1234 and 86312. The server responds with FAIL to all of these.](./imgs/honor-netcat.png "Connecting to honor.pccc on port 61234")
...
1. Open a connection to honor.pccc, and discard the opening message
2. Let code = ""
3. For each digit 0-9
   1. Send (code + digit) to honor.pccc, recording time to send and recv
   ...
   3. If honor.pccc responds with FAIL, continue; otherwise, return (code + digit) as final PIN 
...
This PIN is not our token, however; it does not match our expected 12-character format and the text suggests we would find it in the web directory listing. If we inspect `honor.pccc` in Firefox again, we will see that two new files have been added: `token.txt` and `id_rsa`.

![A screenshot of Firefox with the URL honor.pccc entered. The directory listing now shows three files: "oblivion.jpg", "id_rsa", and "token.txt".](./imgs/honor-second-listing.png "Directory Listing on honor.pccc After PIN")

We will need the `id_rsa` file later for [Question 3](#question-3); if we open `token.txt`, we will find our token.

![A screenshot of Firefox after clicking and opening the token.txt file. As it is a simple text file, it simply displays the text "b6d79f9faa1e".](./imgs/honor-token.png "The Token on Honor.pccc")
...
For the next task, we need to find the token on `Oblivion.pccc`.  As we saw while solving Question 1, there does not appear to be any open ports on `Oblivion.pccc`. However, we did find that image file on `honor.pccc`named `oblivion.jpg`.
...
```bash
curl http://honor.pccc/oblivion.jpg > oblivion.jpg
...
At this point, you can begin inspecting the image for hidden data, such as checking the exif data or viewing the file in an hex editor. In this case, the tool we will need is `steghide`, a popular steganography program. We can use the following command to extract any embedded files using no passcode.
...
The first line of this file appears to be suggesting that there is some sort of UDP communication going on between `phantom.pccc` and `oblivion.pccc`. That idea is supported by the following table documenting an encrypted communication protocol.

Our goal should now be to eavesdrop on the communication between `phantom.pccc` and `oblivion.pccc`.  In this challenge, we do not have access to the `pfSense`, `SecurityOnion`, or other monitoring tools sometimes found in these challenges. Instead, we will need to perform a Man-in-the-Middle (MitM) attack. We don't have access to a Gateway device between the two to intercept their communication, but we don't actually need one since we are on the same local network as them. Instead, we can perform an `ARP spoofing` attack using the `arpspoof` command to convince `oblivion` that we are `phantom` and vice versa. The ARP protocol is used to look up the MAC Address for a given IP Address; the MAC address is the address used to communicate on a local network. The document appears to suggest that `phantom.pccc` initiates the communication, so let's try to spoof `phantom` first. 

First, let's open `Wireshark` and start capturing on the interface `eth1` by clicking the blue shark fin in the top left corner. In the display filter box, type `udp && ip.dst == OBLIVION_IP`, where `OBLIVION_IP` is replaced with the IP address of `oblivion.pccc`. You should not notice any packets incoming at the moment.

![A screenshot of the Wireshark application open in Kali. The interface "eth1" is selected, and the mouse is hovering over the blue shark fin which is labelled "Start capturing packets".](./imgs/oblivion-wireshark.png "Opening Wireshark")

> **Note:** `eth1` is the expected interface on the new PCCC platform. If arpspoof fails, double-check your interface name (e.g., with `ip a`) and substitute it.

Before starting the attack, drop forwarded packets on your Kali box so the kernel doesn't transparently relay the intercepted packets straight on to their real destination — both while you observe in Wireshark and, more importantly, when our relay script later forwards them itself (which would double up the traffic).

```bash
sudo iptables -A FORWARD -j DROP
```

Now let's start the arpspoof attack with the following command, which will start spamming `ARP` packets to `phantom.pccc` that say that the IP Address belonging to `oblivion.pccc` can be found at our MAC Address. With our MAC address in the packets sent by `phantom.pccc`, the local network switch will instead send the packets to us. 

```bash
sudo arpspoof -i eth1 -t phantom.pccc oblivion.pccc
```

Let arpspoof run in the background, and observe Wireshark. After a few seconds, you should begin to receive packets from `phantom.pccc`! To stop the attack, hit `CTRL-C` in the terminal, and hit the red square in Wireshark to stop capturing.
...
Before running the script, note that you will need to provide the MAC address of your Kali machine as an argument (you can find this with `ip a`). You will also need to run the script as root. Also make sure the kernel is still dropping forwarded packets (`sudo iptables -A FORWARD -j DROP`) — the script forwards packets itself, so the kernel must not relay them too.

```bash 
sudo python3 ./phantom_me_oblivion.py {MY_MAC}
```

> **Note:** On the new PCCC platform the relay must recompute packet checksums before forwarding (the script does this by deleting `IP.chksum`/`UDP.chksum` so scapy recalculates them). Packets captured on the virtual/bridged interface carry invalid, offloaded checksums, so forwarding them verbatim makes the receiver silently drop them — the conversation never advances past `Phantom calls`. If you write your own relay, do the same.
...
We can see that `oblivion.pccc` will eventually send over a public key and the token (underlined in red), which is `b42ec1928e9f` in this case.
...
Now all that is left is getting access to `champion.pccc`. To do so, we will check out the private key (the `id_rsa` file) we earned from `honor.pccc` and the public key we earned from `oblivion.pccc`. First, let's check the public key file. If you used the provided script, you'll have a `public_key` file, but since we ran the script as root, that file is owned by root. Change the permissions with `chown` and then print it out.
...

...
X7+zc4Gr5105W1sbAAAAEnVzZXJAY2hhbXBpb24ucGNjYw==
...
We can now try and connect to `champion.pccc`. First, fix the permissions for the key file, then pass it to ssh. Note you may need to type `yes` when you use `ssh` to connect for the first time.

```bash
chmod 600 ./fixed_key
ssh -i ./fixed_key user@champion.pccc
```
...
![A screenshot of the kali terminal after using ssh to connect to champion.pccc. The command ls is run, showing a token.txt file. The command cat is then run, displaying the token value.](./imgs/champion-ssh.png "Connecting to Champion.pccc")

After connecting to `champion.pccc`, we can run `ls` and find a `token.txt`. If we `cat` this file, we will receive the final token! In this case, the token is `9f00c5124c64`.
