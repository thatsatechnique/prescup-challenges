...
SERVER_PORT = 1337
SERVER_HOSTNAME = "oblivion.pccc"
CLIENT_HOSTNAME = "phantom.pccc"
...
def packet_handler(packet):
  ...
  if UDP in packet: # Ensure that this is indeed a UDP packet
    ...
    # Recompute checksums before forwarding. Captured packets on virtual/bridged interfaces
    # often carry invalid (offloaded) checksums; re-sending them verbatim makes the receiver
    # silently drop them. Deleting the fields forces scapy to recalculate them on send.
    del packet[IP].chksum
    del packet[UDP].chksum
    sendp(packet, iface="eth1") # Forward the packet on to its new destination
...
def start_sniffing():
  ...
  interface = "eth1" # The interface for scapy to sniff on
...
if __name__ == "__main__":
  ...
  print(f"Phantom.pccc MAC: {CLIENT_MAC}")
