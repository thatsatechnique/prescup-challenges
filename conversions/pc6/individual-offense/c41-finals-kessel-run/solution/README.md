...
1. a Content-Length/Transfer-Encoding (CL.TE) vulnerability between `channel.pccc` and `maelstrom.pccc`, 
2. a Transfer-Encoding/Content-Length (TE.CL) vulnerability between `maelstrom.pccc` and `maw.pccc`, 
3. and a Carriage Return/Line Feed (CR.LF) vulnerability between `maw.pccc` and `kessel.pccc`. 

All three of these vulnerabilities will need to be combined into a single HTTP request in order to reach the final token on `kessel.pccc`.

![A simple network diagram showing 4 hosts: channel.pccc, maelstrom.pccc, maw.pccc, and kessel.pccc. There is an arrow pointing from channel to maelstrom (labelled /maelstrom), maelstrom to maw (labelled /maw), and maw to kessel (labelled /kessel), representing each of the proxy hops.](../challenge/artifacts/KesselRun.png "Network Diagram for 'Kessel Run'")

A full solution script is available: [sol.py](./sol.py)

## Question 1

*Token 1: Retrieve the token found at `/token` on `maelstrom.pccc` after passing through `channel.pccc`.*

*Enter the token found after accessing /token in the Maelstrom.*

For the first token, we need to access `maelstrom.pccc:8080/token`. Let's begin by taking a look at what we have access to; from the network diagram provided as part of the challenge description, we can visit `channel.pccc:8080/channel` and `channel.pccc:8080/maelstrom`. For now, we can do so using a browser such as Firefox.

![A screenshot of Firefox vising channel.pccc:8080/channel. The text can be found in the file ../challenge/proxies/channel/channel.html.](./imgs/maelstrom-channel.png "Visiting channel.pccc")

![A screenshot of Firefox vising channel.pccc:8080/maelstrom. The text can be found in the file ../challenge/proxies/maelstrom/maelstrom.html.](./imgs/maelstrom-maelstrom.png "Visiting the maelstrom.pccc Through channel.pccc")

Trying to visit `channel.pccc:8080/token` or `maelstrom.pccc:8080/token` does not work. The first option gives us an empty token file on `channel.pccc` and the second fails with "Server Not Found" as it isn't routed through the proxy. We've gained a little big of intel on the system, but we should now pivot to the proxy code provided on `challenge.pccc`.

Visit `challenge.pccc`, download the `proxies_provided.zip` file, and use unzip to extract the code: `unzip proxies_provided.zip`.
We find the following in the zip file:

- directories for each proxy, each of which contain:
  - a proxy.py file (e.g., `channel.py`) that is the Python source code for the `channel.pccc` proxy
...
This is the request as `channel.pccc` will see it, including the newline characters.
...
Since the `Content-Length` is 56, `channel.pccc` will see the following as the body, note there is an extra empty line.
...
That body will then be sent to `maelstrom.pccc` as part of a new request which will look like this.
...
Now `maelstrom.pccc` continues to read from the socket since the `Connection` is set to `keep-alive`, and believes this to be a second request, which it fulfills and sends back to `channel.pccc` (and thus back to us).
...
We can see first the two `Connection` headers and that the proxy says it will close the connection and then also says the connection will remain open. Then we can see that Maelstrom actually handles two requests and in our `netcat` output we can see the "Sample Maelstrom Token File"! Switch to running `netcat` on the real `channel.pccc` now: 

```bash
cat request.http | nc channel.pccc 8080
```

![A screenshot of the exploit being run. The output is the text found under ../challenge/proxies/maelstrom/token.html (however, the TOKEN is replaced with the actual token, af11d8ae6422).](./imgs/maelstrom-token.png "Smuggling to maelstrom.pccc")

In this case, the token was `af11d8ae6422`.

## Question 2

*Token 2: Retrieve the token found at `/token` on `maw.pccc` after passing through `maelstrom.pccc`.*

*Enter the token found after accessing /token in the Maw.*

Before we continue, we will need a reliable way to smuggle requests past `channel.pccc` to make progress towards Kessel. That is, whatever request we need to reach `maw.pccc` will need to take advantage of the CL.TE vulnerability from Token 1. Having to change the content length by hand every time would be tiresome. Using a tool like Burpsuite might be possible, but I don't believe it's well-suited to the multi-layered request smuggling we need to do.
...
def passChannel(innerRequest):
    ...
    request += "Host: channel.pccc:8080\r\n"
...
With that settled, we can now look towards smuggling a packet past `maelstrom` to `maw`. Fortunately, we have actually already discovered the vulnerability! We found that `maelstrom.pccc` had the following code which was really similar to `channel.pccc`.
...
We can do basically the same thing as before, but this time we need to do TE.CL; that is, our first request needs to use `Transfer-Encoding` and our smuggled request will use `Content-Length`. Such a request would look like the following. Note that is 0x30, as the chunk is 48 bytes long (including the `\r\n` after the first line and after the `Host` header). The content length is only 4 bytes; when `maw.pccc` uses the `Content-Length`, it will consume the `30\r\n` and leave the smuggled request. Finally, the first 0 will end the `Transfer-Encoding` when `maw.pccc` parses it.
...
This is the request as `maelstrom.pccc` will see it, including the newline characters.
...
The `maelstrom` proxy begins with the `Transfer-Encoding`, and reads the size of the first chunk is `0x30`. This is the following:
...
The following request is then sent to `maw.pccc`.
...
Since the `Content-Length` is 4, `maw.pccc` will see the following as the body.
...
Now `maw.pccc` begins parsing the encoded chunks, but immediately finds the 0 and stops. That leaves the following request, which `maw.pccc` will parse as normal. 
...
With our `localhost` proxies, we can test this directly and skip `channel.pccc` for now. As in Token 1, save that request to a file and use `netcat` with port `8081`.
...
This means we can now smuggle from `channel.pccc` to `maelstrom.pccc` and from `maelstrom.pccc` to `maw.pccc`. We need to combine these for it to work on the real thing, however. We can use our Python code from earlier for this!
...
unix2dos request.http
python3 buildRequest.py
cat newrequest.http | nc channel.pccc 8080
```

![A screenshot of the exploit being run. The output is the text found under ../challenge/proxies/maw/token.html (however, the TOKEN is replaced with the actual token, 9dc1ddc6ff6e).](./imgs/maw-token.png "Smuggling to maw.pccc")

In this case, the token was `9dc1ddc6ff6e`.

## Question 3

*Token 3: Retrieve the token found at `/token` on `kessel.pccc` after passing through `maw.pccc`.*
...
def passMaelstrom(innerRequest):
    ...
    request += "Host: maelstrom.pccc:8080\r\n"
    request += "Transfer-Encoding: chunked\r\n"
...
13c13
< HOST_ALIASES = ["kessel.pccc", "kessel.pccc:8080"]
```

The proxies have different new line characters; `kessel.pccc` use `\n` while `maw.pccc` (and all the other proxies) use `\r\n`. Consider the following simple request with the new line characters shown:
...
This will be parsed different by the two proxies. The `kessel.pccc` will separate by `\n` (and it then calls the `strip` function in Python to remove any whitespace which includes `\r`), so this works correctly and all three headers will be read and separated correctly.  However, for `maw.pccc`, the `Test` header line will not be correctly parsed. Since `\n` is not recognized as a new line by itself, the headers will be parsed like this by `maw.pccc`.
...
The `Another` header is considered part of the `Test` header. That is, `maw.pccc` believes the value of the `Test` header is `test\nAnother: test`; `kessel.pccc` see three headers, while `maw.pccc` sees online two. We can take advantage of this to sneak in another request. Consider the following request:
...
\n
POST /token HTTP/1.1\n
Host: kessel.pccc:8080\r\n
\r\n
```

This will be parsed by `maw.pccc` as the following, with the incorrect `Test` header. 
...
On the other hand, `kessel.pccc` will parse this with `Test` having a value of just `test`, and the following request will be seen as an entirely separate second request.
...
request += "Host: maw.pccc:8080\r\n"
request += "Connection: keep-alive\r\n"
request += "Test: test\n"
request += "\n"
request += "POST /token HTTP/1.1\n"
request += "Host: kessel.pccc:8080\n"
...
```bash
cat fullRequest.http | nc channel.pccc 8080
```
...
![A screenshot of the exploit being run. The output is the text found under ../challenge/proxies/kessel/token.html (however, the TOKEN is replaced with the actual token, 7365f6becb52).](./imgs/kessel-token.png "Smuggling to kessel.pccc")
