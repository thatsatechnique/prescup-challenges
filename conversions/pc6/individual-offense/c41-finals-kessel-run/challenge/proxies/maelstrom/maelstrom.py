...
HOST_ALIASES = ["maelstrom.pccc", "maelstrom.pccc:8080"]
...
def render_token():
    """Render token.html from token.original.html using the token from the environment"""
    token = os.environ.get("tokenMaelstrom", "")
    with open("./token.original.html", "r") as f:
        html = f.read()
    with open("./token.html", "w") as f:
        f.write(html.replace("TOKEN", token))
