...
- [start.py](./scripts/start.py): The start-up script used by `challenge.pccc`. Uses Python's `Paramiko` module to SSH into `channel.pccc` and place the dynamically generated tokens. As these are mounted Docker volumes, there is no need to restart the containers.
...
First, install Docker. On Ubuntu Server, the original OS used for `channel.pccc`, this can be done with the following commands from [docs.docker.com](https://docs.docker.com/engine/install/ubuntu/):
...
That's all that should be needed. The server should now be accessible in your browser at `channel.pccc`.
